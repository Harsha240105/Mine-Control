...
export const api = {
  ...
  // Server Versions
  getServerVersions: () => request<any>('/server/versions'),
  changeServerVersion: (version: string) =>
  ...
  healthCheck: () =>
    request<any>('/server/health-check', { method: 'POST' }),

  // Server Events
  getServerEvents: (limit = 50) =>
    request<any[]>(`/server/events?limit=${limit}`),
