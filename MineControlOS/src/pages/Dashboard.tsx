...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('player:join', (username: string) => {
      ...
      addEvent('join', username, `${username} joined the game`);
    });
    socket.on('player:leave', (username: string) => {
      toast(`${username} left the game`, { icon: '👋' });
      addEvent('leave', username, `${username} left the game`);
    });
    socket.on('player:chat', (username: string, message: string) => {
      addEvent('chat', username, `<${username}> ${message}`);
    });
    socket.on('player:death', (username: string, message: string) => {
      addEvent('death', username, message);
    });
    return () => {
      ...
      socket.off('player:chat');
      socket.off('player:death');
    };
  }, [socket]);

  const addEvent = (type: string, username: string | null, message: string) => {
    const ev: ServerEvent = { id: Date.now(), type, username, message, details: null, timestamp: Date.now() };
  ...
  const fetchEvents = async () => {
    try {
      const data = await api.getServerEvents(20);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card border border-minecraft-500/20 bg-minecraft-500/5">
        <div className="flex items-start gap-3">
          <Wifi size={18} className="text-minecraft-500 mt-0.5 flex-shrink-0" />
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
              <div className="bg-surface-800 rounded p-2.5">
                <span className="text-gray-500 block mb-0.5">Localhost</span>
                <span className="text-gray-100 font-mono font-medium">localhost:{connectionInfo?.port || '25565'}</span>
              </div>
              <div className="bg-surface-800 rounded p-2.5">
                <span className="text-gray-500 block mb-0.5">LAN IP</span>
                <span className="text-gray-100 font-mono font-medium">{lanIp || 'N/A'}:{connectionInfo?.port || '25565'}</span>
              </div>
              <div className="bg-surface-800 rounded p-2.5">
                <span className="text-gray-500 block mb-0.5">Public IP</span>
                <span className="text-gray-100 font-mono font-medium">{publicIp || 'Fetching...'}:{connectionInfo?.port || '25565'}</span>
              </div>
              <div className="bg-surface-800 rounded p-2.5">
                <span className="text-gray-500 block mb-0.5">Playit.gg</span>
                <span className="text-gray-100 font-mono font-medium">{connectionInfo?.playitAddress || 'N/A'}</span>
              </div>
            </div>
            <p className="text-[11px] text-gray-500 mt-2">
              <strong className="text-gray-400">Local:</strong> Use <code className="text-minecraft-400">localhost</code> on this PC &nbsp;·&nbsp;
              <strong className="text-gray-400">Friends:</strong> Use Playit.gg address (no port forwarding) or Public IP &nbsp;·&nbsp;
              Version: <strong className="text-gray-300">{connectionInfo?.serverVersion || '1.21.1'}</strong>
              {connectionInfo?.onlineMode ? ' · Premium' : ' · Cracked'}
            </p>
          </div>
        </div>
      </div>

      {/* Localhost Binding Warning */}
      {connectionInfo?.boundToLocalhost && (
        <div className="card border border-yellow-500/30 bg-yellow-500/5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-yellow-400 mb-1">Server is bound to localhost</h3>
              <p className="text-xs text-yellow-300/80">
                Your server is configured to listen on 127.0.0.1 only. External connections are blocked.
                Set <code className="text-yellow-400 bg-surface-800 px-1 rounded">server-ip=</code> in server.properties (must be empty).
              </p>
            </div>
          </div>
        </div>
      ...
      {players.length > 0 && (
        <div className="card">
          <h3 className="text-sm font-medium text-gray-200 mb-4 flex items-center gap-2">
            <User size={16} className="text-minecraft-500" />
            Online Players
            <span className="text-xs text-gray-500">({players.length} online)</span>
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {players.map((p) => (
              <div key={p.username} className="bg-surface-800 rounded-lg p-3 border border-surface-700">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-200">{p.username}</span>
                  <span className="text-xs text-gray-500">{p.gamemode}</span>
                </div>
                <div className="space-y-1 text-xs text-gray-400">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{Math.round(p.health)} HP</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Gauge size={11} />
                    <span>{p.ping}ms</span>
        ...
        <div className="card">
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {events.slice(0, 30).map((ev) => (
              <div
                key={ev.id}
                className="flex items-start gap-2 px-2 py-1.5 rounded hover:bg-surface-800/50 transition-colors text-xs"
              >
                <span className="flex-shrink-0 mt-0.5">
                  {ev.type === 'join' && <LogIn size={12} className="text-green-400" />}
                  {ev.type === 'leave' && <LogOut size={12} className="text-yellow-400" />}
                  {ev.type === 'death' && <Skull size={12} className="text-red-400" />}
                  {ev.type === 'chat' && <MessageCircle size={12} className="text-blue-400" />}
                </span>
