...
minecraftServer.on('player:join', (username: string) => {
  ...
  minecraftServer.sendCommand('data get entity ' + username + ' Pos').catch(() => {});
});
...
minecraftServer.on('player:death', (username: string, message: string) => {
  io.emit('player:death', username, message);
});
